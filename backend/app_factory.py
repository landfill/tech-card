"""Shared FastAPI app factory and startup observability."""
import logging
import os
...
class SuppressStatusPollFilter(logging.Filter):
    """Suppress noisy polling access logs for UI polling endpoints."""
...
def build_startup_summary(*, app_role: str = "public", route_labels: list[str] | None = None) -> str:
...
def create_app(
    *,
    ...
    router_specs: list[tuple[object, str, list[str]]],
) -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        if getattr(app.state, "startup_summary_logged", False):
            yield
            return
        logger.info(build_startup_summary(app_role=app_role, route_labels=route_labels))
